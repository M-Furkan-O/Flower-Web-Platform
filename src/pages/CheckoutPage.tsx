...
import type { CartItem, Route, OrderInfo, District } from '../types';
...
type Props = {
  ...
  onPlaceOrder: (
    order: Omit<OrderInfo, 'id' | 'createdAt' | 'status'> & { districtId: number },
  ) => Promise<string>;
};
...
export default function CheckoutPage({
  ...
  const validate = (): boolean => {
    ...
    if (!selectedDistrictId) newErrors.district = 'Teslimat ilçesi seçin';
                ...
                >
                  ...
                  {districts.map((d) => (
                    <option key={d.id} value={d.id}>
                      {d.name} — {d.baseDeliveryFee} TL kargo
                    </option>
                  ))}
                </select>
            ...
            <div className="border-t border-sand-100 pt-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between text-sand-600">
                ...
                <span>
                  {selectedDistrictId
                    ? `${deliveryFee} TL`
                    : <span className="text-sand-400">İlçe seçin</span>}
                </span>
              </div>
